...
//                      }
        ...
        public function deviceSet ($device)
        {
            // dd('run here');
                if ($this->_dState !== SERIAL_DEVICE_OPENED)
                {
                        ...
                        {
                            // dd('run here');
                                // print_r($this->_exec("mode " . $device . " xon=on BAUD=115200"));
                                if (preg_match("@^COM(\d+):?$@i", $device, $matches) and $this->_exec("mode " . $device . " xon=on BAUD=115200") === 0)
                                {
                                    // dd('run here123');
       ...
       public function confCharacterLength ($int)
        ...
        {
                $this->_buffer .= $str;
                // echo $this->_buffer."<br>";
                if ($this->autoflush === true) 
                {
                    // echo "niagi siya dri"."<br>";
                    $this->serialflush();
                }
